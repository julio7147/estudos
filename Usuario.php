<?php
	...
	class Usuario{
		...
		public function atualizarUsuario($nome,$senha,$imagem){
			$sql = MySql::conectar()->prepare("UPDATE `tb_admin.local` SET nome = ?,password = ?,img = ? WHERE user = ?");
		...
		public static function userExists($user){
			$sql = MySql::conectar()->prepare("SELECT `id` FROM `tb_admin.local` WHERE user=?");
		...
		public static function cadastrarUsuario($user,$senha,$imagem,$nome,$cargo){
			$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin.local` VALUES (null,?,?,?,?,?)");
